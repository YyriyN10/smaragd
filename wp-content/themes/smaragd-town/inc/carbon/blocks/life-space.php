		...
		Block::make( 'Life spase' )
		     ->add_tab('Контент блоку', array(
			     ...
			     Field::make_complex('block_list', 'Перелік переваг')
			          ->set_required(true)
			          ->add_fields(array(
			          	Field::make_image('icon', 'Іконка картки')
					          ->set_required(true)
					          ->set_width(20)
										->set_value('url'),
		     ...
		     ->set_render_callback( function ( $fields, $attributes, $inner_blocks ) {
			     ...
			     <?php if( !empty( $fields['title'] ) && !empty( $fields['block_list'] ) ):?>
				     ...
				     <section class="<?php if ( !empty( $fields['block_option_indent_top'] ) ){echo $fields['block_option_indent_top'];}?> <?php if ( !empty( $fields['block_option_indent_bottom'] ) ){echo $fields['block_option_indent_bottom'];}?> animation-tracking"
